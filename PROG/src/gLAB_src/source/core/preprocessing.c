...
void look4interval (TEpoch *epoch, TOptions *options) {
	...
	if ( SR > 0.0 ) {
		...
		//Set the inteval in first epoch as the stimated, so we make sure
...
void prealignSat (TEpoch *epoch, int ind) {
	...
	double					lambda;
	int						ret1,ret2;
	i = satIs(epoch,epoch->sat[ind].PRN,epoch->sat[ind].GNSS);
...
void checkCycleSlips (TEpoch *epoch, TOptions *options, int mode) {
	...
	#pragma omp parallel for private(i,j,system,cycleSlipFound,measCheck,auxMean,auxMean2,samples,meas,index,tdiffNcon,SNR,DataGapCheck,timeDiff,LLICheck,lli,measLLI,Bw,nBwdiff,nBw300diff,nBwdiffThreshold,BwCheck,measBW,C2,Li,LiEst,LiThreshold,LiCheck,measLI,res,LiDiff,L1,C1,L1C1diff,L1C1Threshold,L1C1Check,sigma2,measL1C1,deltaC1,deltaL1,measNcon)
	for ( ii=0;ii<epoch->numSatellites;ii++ ) {

		system = epoch->sat[ii].GNSS;

		LLICheck = 0;		// 0 => no CS detected by LLI detector
							// 1 => CS detected by LLI detector
		DataGapCheck = 0;	// 0 => no CS detected by Data gap detector
							// 1 => CS detected by Data gap detector
		L1C1Check = 0;		// 0 => no CS detected by L1C1 difference detector
							// 1 => CS detected by L1C1 difference detector
		BwCheck = 0;		// 0 => no CS detected by Melbourne-Wubbena detector
							// 1 => CS detected by Melbourne-Wubbena detector
		LiCheck = 0;		// 0 => no CS detected by Geometry-free detector
							// 1 => CS detected by Geometry-free detector

		// Output of the 'checkCycleSlips' function
		cycleSlipFound = 0;
		// Threshold of C1L1 difference detector
		L1C1Threshold = 0;

		measLLI = 0;		// 0 => at least one measurement (L1, C1) is missed or not OK
							// 1 => all measurements (L1 and C1) are OK
		measL1C1 = 0;		// 0 => at least one measurement (L1, C1) is missed or not OK
							// 1 => all measurements (L1 and C1) are OK
		measBW = 0;			// 0 => at least one measurement (L1, C1, L2 and C2) is missed or not OK
							// 1 => all measurements (L1, C1, L2 and C2) are OK
		measLI = 0;			// 0 => at least one measurement (L1, L2) is missed or not OK
							// 1 => all measurements (L1 and L2) are OK
		measNcon = 1;		// 0 => at least one measurement (C1C or C1P) is missed or not OK
							// 1 => all measurements (C1C or C1P) are OK
		measCheck = 0;		// Global measurement check

		// Index of the satellite in the dictionary
		i = satIs(epoch,epoch->sat[ii].PRN,epoch->sat[ii].GNSS);
		// Sanity check for satIs function
		if ( i == -1 ) {
			printError("Problem: satellite is not found. Probably, there are more satellites in view than those defined in MAX_SATELLITES_VIEWED of dataHandling.h", options);
		}
	...
	for ( i = 0; i < options->totalFilterMeasurements; i++ ) {
		if ( options->smoothEpochs != 0 && options->smoothMeas[i] != NA ) {
			for ( j = 0; j < epoch->numSatellites; j++ ) {
				k = satIs(epoch,epoch->sat[j].PRN, epoch->sat[j].GNSS);
				sat = &epoch->sat[j];
...
void sigmaInflation (TEpoch *epoch, TOptions *options) {
	...
	if ( options->smoothEpochs != 0 && options->sigmaInflation == 1 ) {
		...
		for ( j = 0; j < epoch->numSatellites; j++ ) {
			i = satIs( epoch,epoch->sat[j].PRN, epoch->sat[j].GNSS );
