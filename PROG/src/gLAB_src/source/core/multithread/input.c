...
void readRTCM3header (TEpoch *epoch, TEpoch *epochDGNSS, TRTCM3 *rtcm3, TOptions *options) {
	...
	// Receiver name
	strcpy(epochDGNSS->receiver.name,"UNKNOWN");
	...
	// Receiver type
	strcpy(epochDGNSS->receiver.type,"UNKNOWN");
				...
				switch ( msgType ) {
					case 1004:
						...
						for ( j = 0; j < rtcm3->header.gps_no; j++ ) {
							...
							if ( rtcm3->L1prevLLI[ind] == -9999 ) rtcm3->L1prevLLI[ind] = rtcm3->msg1004[j].l1_lock_indicator;
							else if ( rtcm3->msg1004[j].l1_lock_indicator < rtcm3->L1prevLLI[ind] || rtcm3->msg1004[j].l1_lock_indicator == 0 ) {
								rtcm3->L1flagLLI[ind] = 1;
								rtcm3->L1prevLLI[ind] = rtcm3->msg1004[j].l1_lock_indicator;
							}
							if ( rtcm3->L2prevLLI[ind] == -9999 ) rtcm3->L2prevLLI[ind] = rtcm3->msg1004[j].l2_lock_indicator;
							else if ( rtcm3->msg1004[j].l2_lock_indicator < rtcm3->L2prevLLI[ind] || rtcm3->msg1004[j].l2_lock_indicator == 0 ) {
								rtcm3->L2flagLLI[ind] = 1;
								rtcm3->L2prevLLI[ind] = rtcm3->msg1004[j].l2_lock_indicator;
							}
						}
...
int converterRTCM2 (FILE *fd, TRTCM2 *rtcm2, char *fileASCIIcorrections, char *fileASCIIantenna, TEpoch *epoch, TEpoch *epochDGNSS, TOptions *options) {
	...
	if ( options->RTCMmode == ConvertRTCM2 ) {
		fnew = fopen(fileASCIIcorrections, "wa");
		fout = fopen(fileASCIIantenna, "wa");
	}
	...
	while ( rtcm2->i < (READWORDSNO+LEFTLIMIT) && out == 0 ) {
		...
		switch ( rtcm2->my_state ) {
			...
			case EndOneMessage:
				...
				if ( (rtcm2->i + LEFTLIMIT) > rtcm2->l ) rtcm2->my_state = EndOfString;
...
int readRinexObsHeader (FILE *fd, FILE *fdout, TEpoch *epoch, TOptions *options) {
	...
	static int				firstProgressCheck=1;
		...
		if (strncmp(&line[60],"RINEX VERSION / TYPE",20)==0) {
			...
			for (i=0,j=0;i<auxN;i++,j++) {
				...
				if (meas != NA) {
					...
					sprintf(messagestr,"Reading RINEX 2.11 file: Unrecognized measurement type: '%s'",aux);
			...
			for (i=0,j=0;i<epoch->measOrder[actSysObs].nDiffMeasurements;j++,i++) {
				...
				if (meas != NA) {
					...
					sprintf(messagestr,"Reading RINEX 3.00 file: Unrecognized measurement type: '%s'",aux);
					printError(messagestr,options);
					error = 1;
				}				
				obs++;
	...
	while (1) {
		...
		if (len>72 && strncmp(&line[60],"END OF HEADER",13)==0) {
			...
			if (src==RINEX2) {
				...
				dd = atoi(aux);
				getstr(aux,line,9,4);
				//The '.' detection is to avoid some measurements with any 0's to produce a valid date
 ...
 * Description : This function should be called after reading a full epoch.
 *               It updates the satellites' dictionary:
 * Parameters  :
 * Name                           |Da|Unit|Description
 * TEpoch  *epoch                  IO N/A  Structure to save the data
 *****************************************************************************/
void dictionaryUpdate (TEpoch *epoch) {
	int		i, j;
	int		multiConstellationPRN;
	
	// Update list of satellites viewed
	if ( epoch->numSatellites != 0 ) {
		for ( i=0;i<epoch->numSatellites;i++ ) {
			...
			multiConstellationPRN = epoch->sat[i].PRN + epoch->sat[i].GNSS * MAX_SATELLITES_PER_GNSS;
			if ( epoch->satsviewed[0] )
				for ( j=0;j<MAX_SATELLITES_VIEWED;j++ ) {
					if ( multiConstellationPRN == epoch->satsviewed[j] ) {
						satFound = 1;
						satIndex = j;
						break;
					}
			...
			if ( satFound == 1 ) {
				epoch->satsviewed[satIndex] = multiConstellationPRN;
			} else {
				epoch->satsviewed[zeroIndex] = multiConstellationPRN;
			}
		}
	}
}
 ...
 *                                         velocities (0 or 1)
 * int  readClocks                         Flag to read the clocks (0 or 1)
 * int  readingRefFile                     Flag to indicate if we are reading a reference position file (0 or 1)
 * Returned value (int)            O  N/A  Status of the function
 *                                         1 => Properly read
 *                                         2 => Another file concatenated
 *                                         0 => Error
 *                                        -2 => Number of entries in header is incorrect
 *                                        -3 => If reading reference position file, thre is more than
 *                                                one satellite available
 *****************************************************************************/
int readSP3 (FILE *fd, TGNSSproducts *products, int readOrbits, int readClocks, int readingRefFile) {
	char 			line[MAX_INPUT_LINE];
	char    		aux[100];
	int 			len = 0;
	int				properlyRead = 0;
		...
		if (readingHeader) {
			switch(readedLine) {
				case 1:
					...
					if (line[0]!='#') error = 1;
					//versionId = line[1]; //Commented to avoid not used variable warning
				...
				case 2:
					...
					if (readOrbits) products->SP3->orbits.interval = atoi(aux);
					if (readClocks) products->SP3->clocks.interval = atoi(aux);
				...
				case 3:
					...
					if (readOrbits) products->SP3->orbits.numSatellites = products->SP3->orbits.numSatellitesRead = atoi(aux);
					if (readClocks) products->SP3->clocks.numSatellites = products->SP3->clocks.numSatellitesRead = atoi(aux);
					...
					if (readClocks) products->SP3->clocks.block = malloc(sizeof(TSP3clockblock*)*products->SP3->clocks.numSatellites);
					if (readOrbits) auxN = products->SP3->orbits.numSatellites;
					else auxN = products->SP3->clocks.numSatellites;
					for (i=0;i<auxN;i++) {
						if (readOrbits) {
							products->SP3->orbits.block[i] = malloc(sizeof(TSP3orbitblock)*products->SP3->orbits.numRecords);
							for (j=0;j<products->SP3->orbits.numRecords;j++)
								initSP3orbitblock(&products->SP3->orbits.block[i][j]);
						}
						if (readClocks) {
							products->SP3->clocks.block[i] = malloc(sizeof(TSP3clockblock)*products->SP3->clocks.numRecords);
							for (j=0;j<products->SP3->clocks.numRecords;j++) 
			...
			if (line[0]=='*') { // Epoch starting
				...
				usedRecords++;
				if(usedRecords>products->SP3->orbits.numRecords) {
					//Incorrect header. Stop with Error
					return -2;
				} else if(usedRecords==products->SP3->orbits.numRecords) {
					//Save last epoch
					if(readOrbits) memcpy(&products->SP3->orbits.endTime,&t,sizeof(TTime));
					if(readClocks) memcpy(&products->SP3->clocks.endTime,&t,sizeof(TTime));
				} 
				if (usedRecords==1) {
				...
				if (readOrbits) {
					...
					products->SP3->orbits.block[ind][usedRecords-1].x[0] = atof(aux)*1e3;
					getstr(aux,line,18,14);
					products->SP3->orbits.block[ind][usedRecords-1].x[1] = atof(aux)*1e3;
					getstr(aux,line,32,14);
					products->SP3->orbits.block[ind][usedRecords-1].x[2] = atof(aux)*1e3;
					memcpy(&products->SP3->orbits.block[ind][usedRecords-1].t,&t,sizeof(TTime));

					//Read orbit flags
					if(strlen(line)>=80 && strncmp(&line[78],"  ",2)!=0) {
						strncpy(products->SP3->orbits.block[ind][usedRecords-1].orbitflags,&line[78],2);
						products->SP3->orbits.block[ind][usedRecords-1].orbitflags[2]='\0';
					} else {
						products->SP3->orbits.block[ind][usedRecords-1].orbitflags[0]='\0';
					}
				}
				if (readClocks) {
					getstr(aux,line,46,14);
					if (atof(aux)==INVALID_CLOCK) {
						products->SP3->clocks.block[ind][usedRecords-1].clock = INVALID_CLOCK;
					} else {
						products->SP3->clocks.block[ind][usedRecords-1].clock = atof(aux)*(c0/1e6);
					}
					memcpy(&products->SP3->clocks.block[ind][usedRecords-1].t,&t,sizeof(TTime));

					//Read clock flags
					if(strlen(line)>=76 && strncmp(&line[74],"  ",2)!=0) {
						strncpy(products->SP3->clocks.block[ind][usedRecords-1].clockflags,&line[74],2);
						products->SP3->clocks.block[ind][usedRecords-1].clockflags[2]='\0';
					} else {
						products->SP3->clocks.block[ind][usedRecords-1].clockflags[0]='\0';
					}
				}
				...
				if (readOrbits) {
					getstr(aux,line,4,14);
					products->SP3->orbits.block[ind][usedRecords-1].v[0] = atof(aux)*1e-1;
					getstr(aux,line,18,14);
					products->SP3->orbits.block[ind][usedRecords-1].v[1] = atof(aux)*1e-1;
					getstr(aux,line,32,14);
					products->SP3->orbits.block[ind][usedRecords-1].v[2] = atof(aux)*1e-1;
					memcpy(&products->SP3->orbits.block[ind][usedRecords-1].t,&t,sizeof(TTime));
				}
				if (readClocks) {
					getstr(aux,line,46,14);
					products->SP3->clocks.block[ind][usedRecords-1].clockrate = atof(aux)*(c0/1e7);
					memcpy(&products->SP3->clocks.block[ind][usedRecords-1].t,&t,sizeof(TTime));
				}
	...
	while(getL(line,&len,fd)!=-1) {
		...
		if (readingHeader) {
			if (strncmp(&line[60],"RINEX VERSION / TYPE",20)==0) {
				...
				// never be executed...
				if (!found_prn_list) {
					products->SP3->clocks.numSatellites = MAX_SATELLITES_PER_GNSS;
					// Reserve memory for satellites
					products->SP3->clocks.block = malloc(sizeof(TSP3clockblock*)*products->SP3->clocks.numSatellites);
					products->SP3->clocks.vsizeblock = ini_vsize;
					for (i=0;i<products->SP3->clocks.numSatellites;i++) {
					...
					for (j=0;j<MAX_GNSS;j++) {
						for (i=0;i<MAX_SATELLITES_PER_GNSS;i++) {
							products->SP3->clocks.index[j][i] = readedSatellites;
						}
					}
				}
			}
			...
			if ( (aux[0]=='G' || aux[0]=='R' || aux[0]=='E' || aux[0]=='S' || aux[0]=='C' || aux[0]=='J' || aux[0]=='I' ) && atoi(&aux[1])!=0 && strncmp(aux2,"AS",2)==0 )  {
				...
				if (lastT.MJDN != t.MJDN || lastT.SoD != t.SoD) { // New Epoch
					if (products->SP3->clocks.interval == 0 && lastT.MJDN!=0) { // If interval is still not known (and it is not the first epoch)
						products->SP3->clocks.interval = (int)tdiff(&t,&lastT);
					}
				...
				if(strlen(line)>=83 && strncmp(&line[81],"  ",2)!=0) {
			...
			if (strncmp(&line[60],"TYPE / SERIAL NO",16)==0) {
				...
				for (i=0;i<3;i++) {
					...
					if (readingWhat==0) { // Satellite Antenna
						...
						ant.PCO[freqInd][i] = atof(aux)/1e3; // (ANTEX in mm)
					}
				}
...
int readRINEXB (FILE *fd, FILE **fdlist, char **filelist, TSBASdatabox *SBASdatabox, double *rinexVersion, int *prevday, TTime *currentepoch, TOptions  *options) {
	...
	while(getL(line,&len,fd)!=-1) {
						...
						if(currentepoch->MJDN>=0) {
							...
							if (MessageAheadCurrentEpoch==0) {
								...
								if (SBASdatabox->SBASdata[0].missed4msg[sbasblock->PRN]>=4) {
									...
									if (options->GEOindex!=-1 && options->GEOindex!=GEOpos) {
		...
		if(freeblock==1) {	//Free sbasblock only when necessary
			...
			//If we have decimation greater than one second, we still need to handle special events every second
			if(options->decimate>1. && currentepoch->MJDN>=0) {
...
int readSBASmessage (char *binarystring, int messagetype, int *decodedmessagetype, TSBASblock *sbasblock, int *messagesmissing, TOptions  *options) {
	...
	static double 	udreimeterslist[16]={0.75,1.0,1.25,1.75,2.25,3.0,3.75,4.5,5.25,6.0,7.5,15.0,50.0,150.0,-1,-2}; //UDRE (meters)
	static double   udreivariancelist[16]={0.0520,0.0924,0.1444,0.2830,0.4678,0.8315,1.2992,1.8709,2.5465,3.3260,5.1968,20.7870,230.9661,2078.695,-1,-2}; //variance (sigma^2) UDRE (meters^2)

	//Table values for message 7
	static double	fastcorrdegfactorlist[16]={0.00000,0.00005,0.00009,0.00012,0.00015,0.00020,0.00030,0.00045,0.00060,0.00090,0.00150,0.00210,0.00270,0.00330,0.00460,0.00580}; //Fast Correction degradation factor (meters/seconds^2)
	static int 		timeoutintervalnonpreciselist[16]={180,180,153,135,135,117,99,81,63,45,45,27,27,27,18,18};//User Time-Out Interval for fast corrections (seconds). En Route through Nonprecision approach
	static int		timeoutintervalpreciselist[16]={120,120,102,90,90,78,66,54,42,30,30,18,18,18,12,12}; //User Time-Out Interval for fast corrections (seconds). Precision approach mode
	static int  	maxfastcorrupdateintervallist[16]={60,60,51,45,45,39,33,27,21,15,15,9,9,9,6,6}; //Maximum fast correction update interval (seconds)

	//Table values for URA index in message 9
	static double	minurarangelist[16]={0.00,2.40,3.40,4.85,6.85,9.65,13.65,24.00,48.00,96.00,192.00,384.00,768.00,1536.00,3072.00,6144.00}; //Minimun URA range (meters)
	static double	maxurarangelist[16]={2.40,3.40,4.85,6.85,9.65,13.65,24.00,48.00,96.00,192.00,384.00,768.00,1536.00,3072.00,6144.00,-1};	  //Maximum URA range (meters). -1 means no limit (no accuracy)
	static double   uranominalvaluelist[16]={2,2.8,4,5.7,8,11.3,16,32,64,128,256,512,1024,2048,4096,-1}; //URA nominal values. -1 means no accuracy value

	//Table values for Delta UDRE Indicators in message 27
	static double	deltaudrevaluelist[16]={1,1.1,1.25,1.5,2,3,4,5,6,8,10,20,30,40,50,100};	//Delta UDRE values which multiply the UDRE standard deviation in type 2,3,4,5,6 and 24 messages

	//Table values for GIVE in message 26
	static double	givemeterslist[16]={0.3,0.6,0.9,1.20,1.5,1.8,2.1,2.4,2.7,3.0,3.6,4.5,6.0,15.0,45.0,-1}; //GIVE (meters)
	static double	givevariancelist[16]={0.0084,0.0333,0.0749,0.1331,0.2079,0.2994,0.4075,0.5322,0.6735,0.8315,1.1974,1.8709,3.3260,20.7870,187.0826,-1}; //variance (sigma^2) GIVE (meters^2)
	...
	switch(messagetype) {
		...
		case(GEOSATELLITEALMANACS):
			...
			for(i=0;i<sbasblock->numgeoalmanacs;i++) {
				...
				getstr(aux,binarystring,28+i*67,4);				//Extract Health status (only the last three bits)
...
void updateSBASdata (TSBASdata  *SBASdata, TSBASblock  *sbasblock, TTime currentepoch, int messagesmissing, TOptions  *options) {
	...
	if(sbasblock->PRN==SBASdata[0].alarmGEOPRN && sbasblock->messagetype!=DONTUSE) return;

	//Select PRN slots to be updated (the slot with mixed PRN data and the slot with only 1 satellite)
	for(pos=0;pos<=SBASdata[0].numSBASsatellites;pos++) {

		if(pos==0 || pos==SBASdata[0].GEOPRN2pos[sbasblock->PRN]) {
			...
			switch(sbasblock->messagetype) {
				...
				case(DONTUSE):
					...
					if(sbasblock->problems==1) {
						...
						if(pos!=0) {
							if(SBASdata[0].alarmGEOPRN==0 || SBASdata[0].alarmGEOPRN==sbasblock->PRN) {
								SBASdata[0].alarmGEOPRN=SBASdata[pos].PRN;
							} else {
								SBASdata[0].secondalarmGEOPRN=SBASdata[pos].PRN;
							}
						}
						break;
					}
		...
		}//End if pos==0 || pos==SBASdata[0].GEOPRN2pos[sbasblock->PRN]
//	}//End if message!=NULL
/*
*/	
			...
			if (ret) {
				if (word[0]!='#') { // It is not a comment
					...
					if(numline==1) {
						...
						UserError->measType[currenterrortype][UserError->numactiveErrors[currenterrortype]]=measstr2meastype(wordlst[3]);
 ...
 *                                         -3 => SP3 File has incorrect number of entries in header
...
int readReferenceFile (char  *filename, TGNSSproducts *RefPosition, TOptions  *options) {
	...
	if (Posfile==ftSP3) {
		ret=readSP3 (fd, &pastSP3Prod, 1, 0, 1);	
		if (ret==-2) {
			return -3;
		} else if (ret==-3) {
			return -5;
			...
			ret=readSP3 (fd, RefPosition, 1, 0, 1);
			if (ret==-2) {
				return -3;
			} else if (ret==-3) {
				return -5;
				...
				ret=readSP3 (fd, &nextSP3Prod, 1, 0, 1);
				if (ret==-2) {
					return -3;
				} else if (ret==-3) {
					return -5;
...
void readPosFile (FILE  *fd, char  *filename, TGNSSproducts *RefPosition, TOptions  *options) {
	...
	RefPosition->SP3->orbits.Concatblock[0]=NULL;
	RefPosition->SP3->orbits.index[0][0] = 0; //There only be one position for the index (the reference position, which will be stored at 0,0)
	...
	while(getLNoComments(line,&len,fd)!=-1) {
		...
		if (ret) {
			if (word[0]!='#') { // It is not a comment
				...
				RefPosition->SP3->orbits.Concatblock[0] = realloc(RefPosition->SP3->orbits.Concatblock[0],sizeof(TSP3orbitblock)*(usedRecords+1)); //Every orbit position will be for each position read
				initSP3orbitblock(&RefPosition->SP3->orbits.Concatblock[0][usedRecords]);

				//Read epoch
				if (DataFormat==0) {
					//Year DoY SoD X Y Z format
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].t.MJDN=(int)yeardoy2MJDN(atoi(wordlst[YearPos]),atoi(wordlst[DoYPos]),atoi(wordlst[SoDPos]));
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].t.SoD=atof(wordlst[SoDPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[0]=atof(wordlst[XPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[1]=atof(wordlst[YPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[2]=atof(wordlst[ZPos]);
					...
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[0]=PositionXYZ[0];
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[1]=PositionXYZ[1];
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[2]=PositionXYZ[2];
				} else if (DataFormat==2) {
					//GPSWeek SoW X Y Z format
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].t=gpsws2ttime(atoi(wordlst[GPSWeekPos]),atof(wordlst[SoWPos]));
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[0]=atof(wordlst[XPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[1]=atof(wordlst[YPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[2]=atof(wordlst[ZPos]);
					...
					tm.tm_sec  = atoi(aux);
					seconds = atof(aux);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].t.MJDN = MJDN(&tm);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].t.SoD = (double)(tm.tm_hour*3600 + tm.tm_min*60) + seconds;
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[0]=atof(wordlst[XPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[1]=atof(wordlst[YPos]);
					RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[2]=atof(wordlst[ZPos]);
				} else {
				...
				r = sqrt(RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[0]*RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[0] + 
						 RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[1]*RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[1] + 
						 RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[2]*RefPosition->SP3->orbits.Concatblock[0][usedRecords].x[2]);
				...
				if (numvalidline==1) {
					...
					memcpy(&RefPosition->SP3->orbits.startTime,&RefPosition->SP3->orbits.Concatblock[0][usedRecords].t,sizeof(TTime));
				} else {
					//Check that current is epoch is ahead of previous one
					if (tdiff(&RefPosition->SP3->orbits.Concatblock[0][usedRecords].t,&RefPosition->SP3->orbits.Concatblock[0][usedRecords-1].t)<=0.) {
	...
	if (numvalidline==0) {
		...
		memcpy(&RefPosition->SP3->orbits.endTime,&RefPosition->SP3->orbits.Concatblock[0][usedRecords-1].t,sizeof(TTime));
		RefPosition->SP3->orbits.numblocsConcat[0][0]=usedRecords;
