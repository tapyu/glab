...
void PrefitOutlierDetectorAbsolute (TEpoch *epoch, TOptions *options) {
	...
	for(j=0;j<options->totalFilterMeasurements;j++) {
		...
		for(i=0;i<epoch->numSatellites;i++) {
			...
			m=satIs(epoch,epoch->sat[i].PRN,epoch->sat[i].GNSS);
			// Smoothing
			if (options->smoothEpochs!=0 && options->smoothMeas[0]!=NA) {
				ret = getMeasModelValue(epoch,epoch->sat[i].GNSS,epoch->sat[i].PRN,options->smoothMeas[0],&smoothWith,NULL);
				if (ret!=1) {
	...
	for ( i = 0; i < epoch->numSatellites; i++ ) {
		...
		if ( options->SBAScorrections==1 && options->onlySBASiono==0 && (epoch->sat[i].GNSS == GPS || epoch->sat[i].GNSS == GEO)) {
			if (epoch->sat[i].hasSBAScor == 0) {
				printSatSel(epoch,0,"SBAS corrections unavailable",i,options);
				check = 0;
		...
		if ( (epoch->sat[i].elevation < options->elevationMask) && (epoch->receiver.aproxPositionError < 1e5) && (epoch->sat[i].hasOrbitsAndClocks == 1) ) { // 100Km
		...
		if ( options->ionoModel == IONEX && epoch->sat[i].validIonex == 0 ) {
			sprintf(messagestr,"Missing IONEX data");
			printSatSel(epoch,0,messagestr,i,options);
			check = 0;
		} else if ( options->ionoModel == FPPP && epoch->sat[i].validIonex == 0 ) {
		...
		if ( options->onlySBASiono == 1 ) {
			if (epoch->sat[i].hasSBAScor == 0) {
				printSatSel(epoch,0,"SBAS iono unavailable",i,options);
				check = 0;
			}
		}
...
//	}
	...
	// Horizontal and Vertical Dilution Of Precision Computations
	//DOP now is computed before entering this function, so solution is skipped if GDOP is too high
	//calculateDOP(epoch,solution,options);
 ...
 *                                         updated with the solution of the present epoch)
 * TOptions  *options              I  N/A  TOptions structure
 * Returned value (int)            O  N/A  0 => Any of the matrix GtG,GtG_NEU,GtWG are not invertible
 *                                         1 => Everything OK 
 *****************************************************************************/
int calculateDOP (TEpoch *epoch, TFilterSolution *solution, TOptions *options) {
	int		i, j;
	int		res;
	...
	if ( options->SBAScorrections == 1 && options->onlySBASiono == 0 ) {
		...
		if ( options->precisionapproach == PAMODE ) {
			solution->HPL = KHPA * dmajor;
		} else {
			solution->HPL = KHNPA * dmajor;
		}
		solution->VPL = KV * sqrt(VectorGtWG_NEU[5]);
	}
	return 1;
}
...
void StanfordESAComputationRecursive (TEpoch *epoch, char *epochString, int *k, int *kmask, int numsat, int depth, double **G, double *prefits, double *weights, TUnkinfo *unkinfo, TStdESA *StdESA, TOptions  *options) {
	...
	if(res1==0) {
		...
		posx=(unsigned int)(Herror/StdESA->xstep);
		posy=(unsigned int)(HPL/StdESA->ystep);
        ...
        if( (HPL-Herror)>0. && quantError>=quantPL ) {
            ...
            if(StdESA->ystep==StdESA->xstep) {
                ...
                posy=(unsigned int)(quantError/StdESA->ystep)+1;
            }
        }
		if(posy>=StdESA->numpixelver) posy=StdESA->numpixelver-1;
		StdESA->counthor[posy][posx]+=1;
		//Vertical error grid
		posx=(unsigned int)(Verror/StdESA->xstep);
		posy=(unsigned int)(VPL/StdESA->ystep);
		quantError=(double)(posx)*StdESA->xstep;
		quantPL=(double)(posy)*StdESA->ystep;
		if(posx>=(StdESA->numpixelhor-1)) {
			if((VPL-Verror)>0.) {
...
void SBASSummaryDataUpdate (int UseReferenceFile, TEpoch *epoch, TFilterSolution *solution, TSBASdatabox *SBASdatabox, TOptions *options, TUnkinfo *unkinfo) {
	...
	if(solution->HPL<options->HorAlarmLimit && solution->VPL<options->VerAlarmLimit) {
		...
		//Check if LastEpochAvailPrevCurrDisc and FirstEpochAvailAfterPrevDisc are initialized
		if(SBASdatabox->LastEpochAvailPrevCurrDisc.SoD==-1.) {
			//Not initialized, put the current epoch
		...
		if(TimeDiff>SBASdatabox->decimation && RinexInputDataTimeDiff<=SBASdatabox->decimation) {
			...
			if((int)NoDiscontinuityDuration>=options->ContRiskWindowSize) {
				//The stable period without discontinuity was larger than 15 epochs, so we only consider that the last 15 epochs are affected
				SBASdatabox->NumEpochsRisk+=options->ContRiskWindowSize;
			} else {
				//The stable period without discontinuity was smaller than 15 epochs, so we only need to add these epochs (the previous one were already
 ...
 * FILE *fdDiscont                 I  N/A  File descriptor of the output discontinuity file for current hour
 ...
 * TOptions  *options              I  N/A  TOptions structure
 *****************************************************************************/
void calculateSBASAvailability (FILE *fdDiscont, FILE *fdDiscontHour, int LatPos, int LonPos, double latitude, double longitude, TEpoch *epoch, TSBASPlots *SBASplots, TOptions *options) {
    int     	i,ret;
    double  	dmajor, weight;
    double  	RowMatrixG_NEU[4],FalsePrefit[4];
    double  	VectorGtWG_NEU[10];
	double		DiscontinuityDuration; 		//Duration in seconds of the discontinuity
	double		NoDiscontinuityDuration;	//Number of continuous seconds without discontinuity before the current discontinuity
	...
	if(ret==-1) return;	//Matrix could no be computed, so no solution is available
	dmajor = sqrt(0.5 * (VectorGtWG_NEU[2] + VectorGtWG_NEU[0]) + sqrt(0.25 * pow(VectorGtWG_NEU[2] - VectorGtWG_NEU[0], 2) + VectorGtWG_NEU[1] * VectorGtWG_NEU[1]));
	if ( options->precisionapproach == PAMODE ) {
		SBASplots->HPL = KHPA * dmajor;
	} else {
		SBASplots->HPL = KHNPA * dmajor;
	}
	SBASplots->VPL = KV * sqrt(VectorGtWG_NEU[5]);
	...
	if(SBASplots->HPL<options->HorAlarmLimit && SBASplots->VPL<options->VerAlarmLimit) {
		...
		if(options->NoContRiskPlot==0) {
			//Check if LastEpochAvailPrevDisc and LastEpochAvailAfterDisc are initialized
			if(SBASplots->FirstEpochAvailAfterPrevDisc[LatPos][LonPos]==0) {
			...
			if (options->SBASHourlyMaps==1) {
				//Check if LastEpochAvailPrevDiscHourly and LastEpochAvailAfterDiscHourly are initialized
				if(SBASplots->FirstEpochAvailAfterPrevDiscHourly[LatPos][LonPos]==0) {
 ...
 * TOptions  *options              I  N/A  TOptions structure
 * Returned value (int)            O  N/A  Status of the function
 *                                         3 => Not enough satellites for positioning or DOP too high. Try to
 *                                              change mode or GEO in SBAS mode or change to SPP in DGNSS mode
...
int Kalman (TEpoch *epoch, TFilterSolution *solution, int *PRNlist, TUnkinfo *prevUnkinfo, TStdESA *StdESA, TOptions *options) {
	...
	if (isEpochComputable(epoch)) {
		//Calculate DOP
		res = calculateDOP(epoch,solution,options);
		if(res==0) {
			//GGt_NEU or GGt_XYZ or GWGt_NEU are singular matrix. No solution available
			options->printInBuffer = 2;
			printBuffers(epoch, options);
			printBuffersKalman(epoch, options);
			sprintf(messagestr,"%17s Geometry with singular matrix. No solution available",t2doystr(&epoch->t));
			printInfo(messagestr,options);
			return -1;
		}
		if(options->GDOPthreshold == 1 && solution->GDOP > options->maxGDOP) {
			//DOP is over threshold
			epoch->overMaxGDOP=1;
			if ( options->SBAScorrections == 1 && options->onlySBASiono == 0 ) {
				if ((epoch->usableSatellites+epoch->numsatdiscardedSBAS >= 4 ) && epoch->SwitchPossible == 1 ) {
					if ( (options->switchmode == 1 && options->precisionapproach == PAMODE) || options->switchGEO == 1 ) {
						if(options->switchmode == 1 && options->switchGEO == 0) {
							sprintf(messagestr,"%17s GDOP (%.2f) is over threshold (%.2f). Trying to switch to NPA mode",t2doystr(&epoch->t),solution->GDOP,options->maxGDOP);
						} else if ( options->switchmode == 0 && options->switchGEO == 1) {
							sprintf(messagestr,"%17s GDOP (%.2f) is over threshold (%.2f). Trying to switch GEO",t2doystr(&epoch->t),solution->GDOP,options->maxGDOP);
						} else {
							sprintf(messagestr,"%17s GDOP (%.2f) is over threshold (%.2f). Trying to switch GEO or to NPA mode",t2doystr(&epoch->t),solution->GDOP,options->maxGDOP);
						}
						printInfo(messagestr,options);
						// Empty print buffers for SATSEL messages
						options->printInBuffer = 3;
				...
				printBuffers(epoch, options);
				printBuffersKalman(epoch, options);
				sprintf(messagestr,"%17s GDOP (%.2f) is over threshold (%.2f). Skipping epoch",t2doystr(&epoch->t),solution->GDOP,options->maxGDOP);
				printInfo(messagestr,options);
				return -2;
			} else if (options->DGNSS && options->solutionMode != SPPMode) {
				//DGNSS and we can switch to SPP
				...
				printBuffers(epoch, options);
				printBuffersKalman(epoch, options);
				sprintf(messagestr, "Solution moved from DGNSS to SPP at %17s due to a GDOP of %.2f when threshold is %.2f", t2doystr(&epoch->t), solution->GDOP, options->maxGDOP);
				printInfo(messagestr, options);
				return 3;
