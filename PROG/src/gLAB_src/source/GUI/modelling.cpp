...
void gLAB_GUI::on_comboBoxGEOselection_currentIndexChanged(int index) {
    ...
    if ( index == 1 ) ui->checkBoxAllowMixedGEOdata->setChecked(true);
    ...
    *saveString += "###################################################\n\n";
    ...
    if ( ui->labelCurrentTemplate->text() == "SBAS" ||  ui->groupBoxSbas->isChecked()==true ) {
        ...
        if (ui->comboBoxSBASApplication->currentIndex()==1) {
            ...
            if ( ui->checkBoxDisableMT10->isChecked()==true && ui->checkBoxDisableMT2728->isChecked()==true) {
                sbasmaritime=1;
        ...
        if ( ui->comboBoxAirboneReceiverType->currentIndex() == 0 ) {
            if (ui->checkBoxUserDefinedReceiverSigmaMultipathModel->isChecked()==true) {
                ...
                if ( ui->lineEditSbasSigmaMultipathA->text() == "" || ui->lineEditSbasSigmaMultipathB->text() == "" ) {
                    // Parameters a and b
                    *errorString += "MODEL: Parameters a and b for Fixed sigma multipath in [SBAS Options] cannot be empty.\n";
                } else {
                    *saveString += "-model:sigmpath "+ ui->lineEditSbasSigmaMultipathA->text() + " " + ui->lineEditSbasSigmaMultipathB->text() + "\n";
                    *runString << "-model:sigmpath" << ui->lineEditSbasSigmaMultipathA->text() << ui->lineEditSbasSigmaMultipathB->text();
                }
            }
            // Sigma divergence
            ...
            // Sigma noise
            if (ui->checkBoxUserDefinedReceiverSigmaNoise->isChecked()==false) {
       ...
       if ( ui->checkBoxMaintainCurrentGEOafterGEOswitch->isChecked() == true) {
        ...
        if ( ui->lineEditSwitchinCooltime->text() == "" ) {
            *errorString += "MODEL: Switching cooltime in [SBAS Advanced Options] cannot be empty.\n";
            ...
            if ( ui->lineEditSwitchinCooltime->text().toInt() != 20 || WriteAllParameters==1 ) {
