...
typedef struct {
	...
	//Data for summary
	int			TotalEpochs;				//Total epochs processed
	...
	double		HPEPercentileValues;		//95th horizontal error percentile of the values
	double		VPEPercentileValues;		//95th vertical error percentile of the values
...
typedef struct {
	...
	TSP3orbitblock	*Concatblock[MAX_GNSS*MAX_SATELLITES_PER_GNSS];	// Satellite (from index) & Block. These blocks are concatenated from previous and next day and without invalid values
...
typedef struct {
	...
	TSP3clockblock	*Concatblock[MAX_GNSS*MAX_SATELLITES_PER_GNSS];	// Satellite (from index) & Block. These blocks are concatenated from previous and next day and without invalid values
	int				numblocsConcat[MAX_GNSS][MAX_SATELLITES_PER_GNSS];
	int				interval;
} TSP3clocks;
...
typedef struct {
	...
	int		alarmGEOPRN;			//GEO PRN from which we have received an alarm message
	int		secondalarmGEOPRN;		//GEO PRN for an alarm from another GEO PRN when we already are at an alarm state
	int		alarmTimeRemaining;		//Time remaining to be able to use the data from the GEO from which we received the alarm
...
typedef struct {
	...
	double	HPLPercentileValues;			//95th horizontal protection level percentile of the values
	double	VPLPercentileValues;			//95th vertical protection level percentile of the values
...
typedef struct {
	...
	double		HWIR;					//Horizontal worst integrity ratio
	double		VWIR;					//Vertical worst integrity ratio
	int			numpixelhor;			//Number of pixels in horizontal axis
	int			numpixelver;			//Number of pixels in vertical axis
...
typedef struct {
	...
	//int	csLC;						// csLC = 1					=> Use the iono-free carrier-phase combination to detect cycle-slips
									// csLC = 0					=> Do not use the iono-free carrier-phase combination to detect cycle-slips (default)

	//double	csLCmaxjump;			// Maximum jump threshold between two consecutive measured LC values (default 1)

	//double	csLCmax;				// Maximum threshold between estimated and measured LC values (default 0.08)

	//double	csLCt;					// Time constant to set the threshold between maximum and minimum (default 60)

	//int	csLCsamples;				// Minimum number of epochs needed to fit the second-degree polynomial for LC (default 6)
	...
	double	sigma2multipath[2];			// Stores parameters a,b for sigma multipath, being sigma=a+b*e^(-satelevation/10º). Values saved in meters
	...
	int	percentile;						//Percentile value for computing the error percentile (default 95)
	...
	int		ContRiskWindowSize;		//Size of the sliding window for the continuity risk (default 15 epochs)
	...
	int	GDOPthreshold;		// 0 => Disables the GDOP threshold
							// 1 => Enables the GDOP threshold (default)

	double	maxGDOP;		// Maximum GDOP allowed when using DGNSS, in metres (default 30.0 m)
