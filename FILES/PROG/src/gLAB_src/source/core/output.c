...
/*	if (options->csLC) {
		fprintf(options->outFileStream,"INFO PREPROCESSING Cycle Slip LC:    ON  [MaxJump:%.3lf  Max:%.3lf  To:%.3lf  Samples:%d]\n",options->csLCmaxjump,options->csLCmax,options->csLCt,options->csLCsamples);
	} else {
		fprintf(options->outFileStream,"INFO PREPROCESSING Cycle Slip LC:    OFF\n");
	}*/
	...
	if (options->SBAScorrections == 1 && options->onlySBASiono == 0 ) {
		if ( options->useDatasummary == 1 ) {
			fprintf(options->outFileStream,"INFO SBAS summary parameters: HAL: %.2f metres VAL: %.2f metres Percentile: %2d\n",options->HorAlarmLimit,options->VerAlarmLimit,options->percentile);
		} else {
			fprintf(options->outFileStream,"INFO Receiver position not fixed. SBAS summary will not be generated\n");
		}
	}
...
void printInput (TEpoch *epoch, TTGDdata *tgdData, TOptions *options) {
	...
	if (options->printInput) {
		for (i=0;i<epoch->numSatellites;i++) {
			k = satIs(epoch,epoch->sat[i].PRN,epoch->sat[i].GNSS);
...
void printOutput (TEpoch *epoch, TEpoch *epochDGNSS, TFilterSolution *solution, TOptions *options, TUnkinfo *unkinfo, char *Obsfilename, char *version) {
	...
	if (options->fdkml!=NULL || options->fdkml0!=NULL) {
		...
		//if((options->DGNSS && options->solutionMode==DGNSSMode) || options->DGNSS==0) {
			...
			//Save data
			epoch->KMLData[epoch->numKMLData][0]=positionNEU[1]/d2r;
			epoch->KMLData[epoch->numKMLData][1]=positionNEU[0]/d2r;
	...
	if (options->fdkml!=NULL) {
		...
		//if((options->DGNSS && options->solutionMode==DGNSSMode) || options->DGNSS==0) {
		fprintf(options->fdkml,"          %.9f,%.9f,%.9f\n",positionNEU[1]/d2r,positionNEU[0]/d2r,positionNEU[2]);
...
void printSBASUNSEL (TEpoch  *epoch, int  satIndex, int errorcode, TSBASdata *SBASdata, TSBAScorr *SBAScorr, TOptions  *options) {
	...
	if ( options->GEOindex == -1 ) GEOPRN = -1;
	...
	switch(errorcode) {
		case 4:
			...
			if ( GEOPRN == 0 ) {
				sprintf(GEOtext,"GEO %3d",SBASdata[0].alarmGEOPRN);
			} else {
				sprintf(GEOtext,"current GEO");
			}
			if ( SBASdata[0].alarmTimeRemaining == 1 ) {
				sprintf(errorstr[4],"Received alarm message for %s at epoch %17s. Time remaining to finish alarm: %2d second",GEOtext,t2doystr(&SBASdata[0].alarmtime),SBASdata[options->GEOindex].alarmTimeRemaining);
			} else {
				sprintf(errorstr[4],"Received alarm message for %s at epoch %17s. Time remaining to finish alarm: %2d seconds",GEOtext,t2doystr(&SBASdata[0].alarmtime),SBASdata[options->GEOindex].alarmTimeRemaining);
			}
			break;
		case 5:
	...
	switch ( rtcm2->header.message_type ) {
		case 1:
			...
			if ( options->RTCMmode == ProcessRTCM2 ) {
				...
				for ( i=0;i<ns;i++ ) {
					j = satIs(epochDGNSS,epochDGNSS->sat[i].PRN,epochDGNSS->sat[i].GNSS);
	...
	if(mode==0) {
		...
		if(options->KMLTimeStamps==1) {
			...
			for(i=0;i<epoch->numKMLData;i++) {
				...
				fprintf(fd,"        <coordinates>\n");
				if(heightmode==0) {
					fprintf(fd,"          %.9f,%.9f,%.9f\n",epoch->KMLData[i][0],epoch->KMLData[i][1],epoch->KMLData[i][2]);
				} else {
					fprintf(fd,"          %.9f,%.9f,0.00\n",epoch->KMLData[i][0],epoch->KMLData[i][1]);
				}
				fprintf(fd,"        </coordinates>\n");
				if(heightmode==0) {
					fprintf(fd,"        <altitudeMode>absolute</altitudeMode>\n");
					...
					for(i=0;i<epoch->receiver.numRecStation-1;i++) {
						...
						for(;j<epoch->numKMLData;j++) {
							if(tdiff(&epoch->KMLTime[j],&epoch->receiver.ChangedStaEpoch[i])>=0.) {
								...
								fprintf(fd,"      <Point>\n");
								fprintf(fd,"        <coordinates>\n");
								fprintf(fd,"          %.9f,%.9f,%.9f\n",epoch->KMLData[j][0],epoch->KMLData[j][1],epoch->KMLData[j][2]);
								fprintf(fd,"        </coordinates>\n");
								if(heightmode==0) {
									fprintf(fd,"        <altitudeMode>absolute</altitudeMode>\n");
								}
...
void writeSP3file(FILE *fd, TEpoch  *epoch, TFilterSolution  *solution, TUnkinfo  *unkinfo, int EndofFile, char *version, TOptions  *options) {
	...
	if (header==0) {
		...
		fprintf(fd,"## %4d %15.8f %14.8f %5d %15.13f\n",GPSWeek,SoW,options->decimate,epoch->t.MJDN,epoch->t.SoD/86400);
 ...
 * TOptions  *options              I  N/A  TOptions structure
 *****************************************************************************/
void writeSBASAvailFile (FILE *fdAvail, FILE *fdRisk, FILE *fdDiscont, TEpoch  *epoch, TSBASPlots *SBASplots, int totalAvailabilityEpochs, int hour, TOptions *options) {
	...
	if (hour==-1) {
		...
		fprintf(fdAvail," %6.2f %6.2f %7.2f %7.2f %10.2f %10.2f %6.2f %6.2f %03d %4d %9d %7d %9.2f\n",options->minLatplots,options->maxLatplots,options->minLonplots,options->maxLonplots,options->AvailabilityPlotStep,options->SBASPlotsRecHeight,options->HorAlarmLimit,options->VerAlarmLimit,(int)DoY,Year,totalAvailabilityEpochs,GEOused,options->elevationMask*r2d);
	} else {
		fprintf(fdAvail,"#MINLAT MAXLAT  MINLON  MAXLON RESOLUTION REC-HEIGHT    HAL    VAL DoY YEAR NUMEPOCHS GEO-PRN ELEV-MASK HOUR\n");
		fprintf(fdAvail," %6.2f %6.2f %7.2f %7.2f %10.2f %10.2f %6.2f %6.2f %03d %4d %9d %7d %9.2f   %02d\n",options->minLatplots,options->maxLatplots,options->minLonplots,options->maxLonplots,options->AvailabilityPlotStep,options->SBASPlotsRecHeight,options->HorAlarmLimit,options->VerAlarmLimit,(int)DoY,Year,totalAvailabilityEpochs,GEOused,options->elevationMask*r2d,hour);
	}
	...
	if(options->NoContRiskPlot==0) {
		if (hour==-1) {
			...
			fprintf(fdRisk," %6.2f %6.2f %7.2f %7.2f %10.2f %10.2f %6.2f %6.2f %03d %4d %9d %7d %9.2f %11d\n",options->minLatplots,options->maxLatplots,options->minLonplots,options->maxLonplots,options->AvailabilityPlotStep,options->SBASPlotsRecHeight,options->HorAlarmLimit,options->VerAlarmLimit,(int)DoY,Year,totalAvailabilityEpochs,GEOused,options->elevationMask*r2d,options->ContRiskWindowSize);
		} else {
			...
			fprintf(fdRisk," %6.2f %6.2f %7.2f %7.2f %10.2f %10.2f %6.2f %6.2f %03d %4d %9d %7d %9.2f %11d   %02d\n",options->minLatplots,options->maxLatplots,options->minLonplots,options->maxLonplots,options->AvailabilityPlotStep,options->SBASPlotsRecHeight,options->HorAlarmLimit,options->VerAlarmLimit,(int)DoY,Year,totalAvailabilityEpochs,GEOused,options->elevationMask*r2d,options->ContRiskWindowSize,hour);
		}
			...
			if(options->NoContRiskPlot==0) {
				...
				if (hour==-1) {
					...
					if(SBASplots->SBASNumEpochsAvail[latPos][lonPos]!=0) {
						fprintf(fdRisk,"%6.2f %7.2f %14.1E %7d %9d\n",
								latitude,
								longitude,
						...
						fprintf(fdRisk,"%6.2f %7.2f %14.1f %7d %9d\n",
								latitude,
								longitude,
								1.,
								0,
					...
					if(SBASplots->LastEpochAvailPrevCurrDiscHourly[latPos][lonPos]!=(double)(hour*60+3599)) {
						//There has been a discontinuity
						DiscontinuityDuration=epoch->t.SoD-SBASplots->LastEpochAvailPrevCurrDiscHourly[latPos][lonPos];
						...
						fprintf(fdDiscont,"%6.2f %7.2f %10.1f %9.1f %8.1f %13.1f\n",latitude,longitude,SBASplots->LastEpochAvailPrevCurrDiscHourly[latPos][lonPos]+1.,epoch->t.SoD,DiscontinuityDuration,NoDiscontinuityDuration);
					}
...
void printSBASSummary (int UseReferenceFile, TEpoch *epoch, TSBASdatabox *SBASdatabox,  TStdESA *StdESA, TOptions *options) {
	...
	double		PAPercentage,NPAPercentage,PALimitsPercentage,RiskVal;
	double		NoDiscontinuityDuration;	//Number of continuous seconds without discontinuity before the current discontinuity
	double		MIEpochsPercentage,HMIEpochsPercentage,VMIEpochsPercentage;
	double		FewSatEpochsPercentage,SingularEpochsPercentage,GDOPEpochsPercentage;
	...
	if(SBASdatabox->TotalPAEpochs==0) {
		...
		SBASdatabox->HPLPercentileValues=0;
		SBASdatabox->VPLPercentileValues=0;
		SBASdatabox->HWIR=0.;
		SBASdatabox->VWIR=0.;
	} else {
		if (epoch->TotalEpochsRef==0) {
			...
			if (options->recPosProvided==1) {
				percentileSamples=(int)(epoch->TotalEpochsRef*.95);
		...
		percentileSamples=(int)(SBASdatabox->TotalPAEpochs*.95);
	...
	if (options->firstEpochSummary.MJDN!=-1 && epoch->StartSummaryEpoch.MJDN==-1) {
		...
		t2doy(&epoch->t,&year,&DoY);
		fprintf(options->outFileStream,"INFO Last  epoch of summary: %02d/%02d/%4d %02d:%02d:%02d.%02d / %s / %4d %9.2f\n",tm.tm_mday,tm.tm_mon+1,year,tm.tm_hour,tm.tm_min,(int)(SecondsComp+.00005),(int)((SecondsComp-((double)((int)SecondsComp)))*1000.+0.00005),t2doystr(&epoch->t),GPSWeek,SoW);
	}
	...
	fprintf(options->outFileStream,"INFO Total epochs skipped (any reason): %d ( %.3f%% )\n",epoch->NumNoRefSolSkipped+epoch->NumNoSatSkipped+epoch->NumSingularMatrixSkipped+epoch->NumGDOPSkipped,NoRefSolEpochPercentage+FewSatEpochsPercentage+SingularEpochsPercentage+GDOPEpochsPercentage);

	//Print Continuity Risk
	if (RiskVal==0. || RiskVal==1.) {
		fprintf(options->outFileStream,"INFO Continuity Risk (%d epochs sliding window): %d\n",options->ContRiskWindowSize,(int)RiskVal);
	} else {
		fprintf(options->outFileStream,"INFO Continuity Risk (%d epochs sliding window): %.4E\n",options->ContRiskWindowSize,RiskVal);
	}

    if(options->recPosProvided==1) {
		if (epoch->TotalEpochsRef==0) {
				fprintf(options->outFileStream,"INFO No epochs with solution %sfor computing MIs%s percentiles%s\n",UseReferenceFile==1?"and position from reference file ":"",options->stanfordesa==1?",":" and",options->stanfordesa==1?" and Stanford-ESA statistics":"");
		} else {
			//Get GPSWeek of start epoch
	...
	if(SBASdatabox->TotalPAEpochs!=0) {
		if(options->recPosProvided==1 && epoch->TotalEpochsRef!=0) {
			...
			fprintf(options->outFileStream,"INFO Horizontal %3d Positioning Error Percentile: %6.2f metres\n",options->percentile,epoch->HPEPercentileSamples);

			//Print Vertical error Percentile
			fprintf(options->outFileStream,"INFO Vertical   %3d Positioning Error Percentile: %6.2f metres\n",options->percentile,epoch->VPEPercentileSamples);
		}

		//Print Horizontal protection level Percentile
		fprintf(options->outFileStream,"INFO Horizontal %3d Protection  Level Percentile: %6.2f metres\n",options->percentile,SBASdatabox->HPLPercentileSamples);

		//Print Vertical protection level Percentile
		fprintf(options->outFileStream,"INFO Vertical   %3d Protection  Level Percentile: %6.2f metres\n",options->percentile,SBASdatabox->VPLPercentileSamples);
	...
	fprintf(options->outFileStream,"INFO Station: %4s Lon: %12.8f Lat: %13.8f Height: %9.4f HWIR: %7.4f VWIR: %7.4f MIs: %3d Hor_MIs: %3d Ver_MIs: %3d HPE_Percentile: %3d %6.2f VPE_Percentile: %3d %6.2f MaxHPE: %6.2f MaxVPE: %6.2f HPL_Percentile: %3d %6.2f VPL_Percentile: %3d %6.2f MaxHPL: %6.2f MaxVPL: %6.2f Avail%%: %7.3f Cont_Risk: %6.4E HWIR_ESA: %7.4f VWIR_ESA: %7.4f MIs_ESA: %4d Hor_MIs_ESA: %4d Ver_MIs_ESA: %4d\n",
			 receiverid,
 ...
 * TOptions  *options              I  N/A  TOptions structure
 *****************************************************************************/
void printDGNSSSummary (int UseReferenceFile, TEpoch *epoch, TStdESA *StdESA, TOptions *options) {
	...
	//Print total number of epochs skipped
	fprintf(options->outFileStream,"INFO Total epochs skipped (any reason): %d ( %.3f%% )\n",epoch->NumNoSatSkipped+epoch->NumSingularMatrixSkipped+epoch->NumGDOPSkipped,FewSatEpochsPercentage+SingularEpochsPercentage+GDOPEpochsPercentage);

    if(options->recPosProvided==1) {
		if (epoch->TotalEpochsRef==0) {
				fprintf(options->outFileStream,"INFO No epochs with DGNSS solution %sfor computing percentiles%s\n",UseReferenceFile==1?"and position from reference file ":"",options->stanfordesa==1?" and Stanford-ESA statistics":"");
		} else {
			//Get GPSWeek of start epoch
			ttime2gpswsnoroll(&epoch->StartSummaryPercentileEpoch, &GPSWeek, &SoW);
			//Compute calendar date of start epoch
	...
	fprintf(options->outFileStream,"INFO Rover: %4s Lon: %12.8f Lat: %13.8f Height: %9.4f HPE_Percentile: %3d %6.2f VPE_Percentile: %3d %6.2f MaxHPE: %6.2f MaxVPE: %6.2f Avail%%: %7.3f\n",
			 receiverid,
			 epoch->receiver.aproxPositionNEU[1]*r2d,
			 epoch->receiver.aproxPositionNEU[0]*r2d,
			 epoch->receiver.aproxPositionNEU[2],
...
void printSummary (int UseReferenceFile, TEpoch *epoch, TStdESA *StdESA, TOptions *options) {
    ...
    if(options->recPosProvided==1) {
		if (epoch->TotalEpochsRef==0) {
				fprintf(options->outFileStream,"INFO No epochs with solution %sfor computing percentiles%s\n",UseReferenceFile==1?"and position from reference file ":"",options->stanfordesa==1?" and Stanford-ESA statistics":"");
		} else {
			//Get GPSWeek of start epoch
			ttime2gpswsnoroll(&epoch->StartSummaryPercentileEpoch, &GPSWeek, &SoW);
			//Compute calendar date of start epoch
	...
	// Print percentile, station name and geodetic latitude and longitude. This line is useful for making maps
	fprintf(options->outFileStream,"INFO Station: %4s Lon: %12.8f Lat: %13.8f Height: %9.4f HPE_Percentile: %3d %6.2f VPE_Percentile: %3d %6.2f MaxHPE: %6.2f MaxVPE: %6.2f Avail%%: %7.3f\n",
			 receiverid,
			 epoch->receiver.aproxPositionNEU[1]*r2d,
			 epoch->receiver.aproxPositionNEU[0]*r2d,
			 epoch->receiver.aproxPositionNEU[2],
